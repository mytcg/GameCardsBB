...
public final class ThumbnailField extends Field {
	...
	public void paint(Graphics g) {
		...
		g.drawBitmap(Const.getWidth() - (button_thumbnail.getWidth() + 5), 2, button_thumbnail.getWidth(), getPreferredHeight(), button_thumbnail, 0, 0);
		
		g.setColor(Const.FONTCOLOR);
		if (focus) {
			g.setColor(Const.SELECTEDCOLOR);
			g.drawBitmap(0, 0, getPreferredWidth(), getPreferredHeight(), button_sel_right_edge, 0, 0);
		}
		Font _font = getFont();
		...
		if (card.getUpdated() == 1) {
			g.drawText("*" +label1, (int)((double)(Const.getWidth()-(button_thumbnail.getWidth()+_font.getAdvance(label1)))/2), 4);
		} else {
			g.drawText(label1, (int)((double)(Const.getWidth()-(button_thumbnail.getWidth()+_font.getAdvance(label1)))/2), 4);
		}
