...
public class ShareScreen extends AppScreen implements FieldChangeListener
{
	...
	private String message = "Go to http://prg.mytcg.net/ to download the Paul Roos Mobile Application.";
	private String smsnumber = "";
	
	public ShareScreen(AppScreen screen)
	{
		super(screen);
	...
	public void fieldChanged(Field f, int i) {
		if (f == exit) {
			...
			if ((number.getText() == null)||(number.getText().length() <= 0)) {
				...
				doConnect(Const.trade+Const.trademethod+number.getText(), false);
			}
		}
	}
