...
import net.mytcg.fhm.ui.custom.ColorLabelField;
import net.mytcg.fhm.ui.custom.FixedButtonField;
import net.mytcg.fhm.ui.custom.ListItemField;
import net.mytcg.fhm.ui.custom.PageNumberField;
import net.mytcg.fhm.ui.custom.ThumbnailField;
import net.mytcg.fhm.util.Const;
import net.mytcg.fhm.util.Product;
		...
		if ((!(isDisplaying()))||(update)) {
	    	...
	    	if ((fromIndex = val.indexOf(Const.xml_result)) != -1) {
	    		...
	    		if ((fromIndex = val.indexOf(Const.xml_premium)) != -1) {
    				String credits = val.substring(fromIndex+Const.xml_premium_length, val.indexOf(Const.xml_premium_end, fromIndex));
    				_instance = SettingsBean.getSettings();
    				_instance.setCredits(credits);
    				SettingsBean.saveSettings(_instance);
    				synchronized(UiApplication.getEventLock()) {
    					header.setText("Premium Credits: " + SettingsBean.getSettings().getCredits());
    	    		}
