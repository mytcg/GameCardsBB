...
public final class MenuThumbnailField extends LabelField {	
	...
	private Bitmap button_thumbnail_sel;
	private Bitmap main_image;
	private Bitmap button_select;
	MenuField menu;
	
	public MenuThumbnailField(Bitmap menuOption, Bitmap menuOptionSel, Bitmap mainImage, MenuField menu) {
		button_thumbnail = menuOption;
		button_thumbnail_sel = menuOptionSel;
	...
	public void paint(Graphics g) {
		...
		if (focus) {
			//g.drawBitmap(2, 7, button_select.getWidth(), button_select.getHeight(), button_select, 0, 0);
			g.drawBitmap(6, 0, button_thumbnail_sel.getWidth(), button_thumbnail_sel.getHeight(), button_thumbnail_sel, 0, 0);
		} else{	
